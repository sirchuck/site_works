...
";
		...
		if($this->debugMode){
			...
			foreach($this->tool->listFiles(SITEWORKS_DOCUMENT_ROOT . '/dev/_css/themes/',2,false) as $v){
				...
				if (!is_dir($new_path)){mkdir($new_path,0775,true);}
				$tmp2 = preg_replace(['/root_asset_url/','/base_asset_url/','/public_asset_url/','/asset_url/','/base_url/','/root_url/','/public_url/'],[$this->uri->root_asset_url,$this->uri->base_asset_url,$this->uri->public_asset_url,$this->uri->asset_url,$this->uri->base_url,$this->uri->root_url,$this->uri->public_url],$tmp2);
