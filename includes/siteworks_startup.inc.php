...
";
		...
		if($this->debugMode){
			...
			foreach($this->tool->listFiles(SITEWORKS_DOCUMENT_ROOT . '/dev/_css/themes/',2,false) as $v){
				...
				foreach($this->sw_pass as $sk => $sv){ $tmp4 = '/sw_pass['. $sk . ']/'; $tmp2 = preg_replace($tmp4,$sv,$tmp2); }
			...
			foreach($this->tool->listFiles(SITEWORKS_DOCUMENT_ROOT . '/dev/_js/themes/',2,false) as $v2){
				...
				foreach($r->f as $k => $v){
					...
					if( substr( $k, 0, 3 ) !== "sw_" ){
						...
						$tmp3 = preg_replace(["/\r\n|\r|\n/",'/"/','/asset_url/'],["",'\"',$this->uri->asset_url],$tmp3);
						foreach($this->sw_pass as $sk => $sv){ $tmp4 = '/sw_pass['. $sk . ']/'; $tmp3 = preg_replace($tmp4,$sv,$tmp3); }
