...
class siteworks_startup
{
    ...
    public function __construct(){
    	...
    	if($this->cookie_samesite!=''){ini_set('session.cookie_samesite',$this->cookie_samesite);}
		...
		$debug_out = '';
		if($this->debugMode){
			$projectname = $this->project_name . ' : ' . (isset($_SERVER['REMOTE_ADDR']) ? gethostbyname($_SERVER['REMOTE_ADDR']) : '');
...
";
		...
		if($this->debugMode && $this->debugBuild){
			...
			foreach($this->tool->listFiles(SITEWORKS_DOCUMENT_ROOT . '/dev/_css/themes/',2,false) as $v){
				...
				if (!is_dir($new_path)){mkdir($new_path,0775,true);}
				$tmp2 = preg_replace(['/root_asset_url/','/base_asset_url/','/asset_url/','/base_url/','/root_url/'],[$this->uri->root_asset_url,$this->uri->base_asset_url,$this->uri->asset_url,$this->uri->base_url,$this->uri->root_url],$tmp2);
			...
			if( $this->useUnitTests ){
				$tmp='';$tmp2='';$tmp3='';$wf=false;$tc=0;$tt=[];
				foreach($this->tool->listFiles(SITEWORKS_DOCUMENT_ROOT . '/dev/',1,true) as $vf){
					if( stripos(strrev($vf['name']), 'php.') === 0 ){
						preg_match_all('/(?<=\#_sw>)(.*?)(?=\n)|(?<nb>[^\{\}]*){0}(function.*?)(?<bb>\{\g<nb>\}|\{(?:\g<nb>\g<bb>\g<nb>)*\})/', file_get_contents($vf['path'].$vf['name']), $m);
						foreach($m[0] as $v){
							if(strtolower(substr($v,0,8))=='function'){
						        ...
						        $wf = true;
								$tt[$tc][0][] = trim($v);
								$tt[$tc][4] = str_replace(SITEWORKS_DOCUMENT_ROOT . '/dev/','',$vf['path'].$vf['name']);
							}
						}
					} // End if php file
				} // End foreach file
...
'.$tmp2.'
			...
			$this->_tmp .= "Testing errror: " . $e->getMessage() . "\n";
		}
		$this->_tmp .= "\nTotal Unit Tests: " . $this->_sw_test_counter;
		if($this->_sw_test_counter_fail == 0){
			$this->_tmp .= "\n[c_green]Total Unit Test Failures: " . $this->_sw_test_counter_fail . "[c_clear]\n\n";
		}else{
			$this->_tmp .= "\n[c_red]Total Unit Test Failures: [c_light_red]" . $this->_sw_test_counter_fail . "[c_clear]\n\n";
		}
		return $this->_tmp;
	}
'.$tmp3.'
